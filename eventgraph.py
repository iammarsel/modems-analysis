    ...
    try:
        ...
        with open(filename) as f:
            for line in f.read().splitlines():
                ...
                modulation = line[9]
                slot_num = line[5]
                if (line[10].strip() in mapping and len(slot_num) == 6 and
                    date_time_obj.month == int(month_val) and date_time_obj.year == int(year_val) and ((checkmarks['TX'].get() == 1
                                                                                                        and modulation == 'TX Modulation') or (checkmarks['RX'].get() == 1 and modulation == 'RX Modulation'))):
    ...
    for i in range(3):
        ...
        if len(graph_info[i][0]) > 0 and len(graph_info[i][1][1]) > 0:
            ...
            axs[i].title.set_text(
                f'{titles[temp1]} Total time under {threshold.get()} Mb/s {str(graph_info[i][1][2])} seconds')
            ...
            axs[i].plot(graph_info[i][1][0], graph_info[i][0]
                        [0], label=f'Modem {temp1} Speed')
            axs[i].plot(graph_info[i][1][0], graph_info[i][0]
                        [1], label=f'Modem {temp2} Speed')
            if temp3 != '':
                axs[i].plot(graph_info[i][1][0], graph_info[i]
                            [0][2], label=f'Modem {temp3} Speed')
            axs[i].plot(graph_info[i][1][0], graph_info[i]
...
g1 = CTkEntry(master=window, placeholder_text='Enter modems (seperated by commas)',
              width=120, height=25, border_width=2, corner_radius=10)
g1.place(relx=0.5, rely=0.6, anchor=CENTER)

g2 = CTkEntry(master=window, placeholder_text='Enter modems (seperated by commas)',
              width=120, height=25, border_width=2, corner_radius=10)
g2.place(relx=0.5, rely=0.7, anchor=CENTER)

g3 = CTkEntry(master=window, placeholder_text='Enter modems (seperated by commas)',
              width=120, height=25, border_width=2, corner_radius=10)
g3.place(relx=0.5, rely=0.8, anchor=CENTER)

threshold = CTkEntry(master=window, placeholder_text='Enter threshold',
                     width=120, height=25, border_width=2, corner_radius=10)
